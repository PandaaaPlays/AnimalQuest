...
public class ShopManager implements org.bukkit.event.Listener {
        ...
        private final org.bukkit.NamespacedKey shopKey;

        public ShopManager() {
                this.shops = new HashMap<>();
                this.shopKey = new org.bukkit.NamespacedKey(ca.pandaaa.animalquest.AnimalQuest.getPlugin(), "shop");
                initShops();
        }
        ...
        public void createShop(String name, String displayName) {
                shops.put(name.toLowerCase(), new Shop(displayName));
        }
        ...
        public void summonShop(Player player, String shopName) {
                Shop shop = getShop(shopName);
                if (shop != null) {
                        org.bukkit.entity.Villager npc = (org.bukkit.entity.Villager) player.getWorld()
                                        .spawnEntity(player.getLocation(), org.bukkit.entity.EntityType.VILLAGER);
                        npc.setCustomName(ca.pandaaa.animalquest.utils.Utils.applyFormat(shop.getName()));
                        npc.setCustomNameVisible(true);
                        npc.setAI(false);
                        npc.setInvulnerable(true);
                        npc.setSilent(true);
                        npc.setProfession(org.bukkit.entity.Villager.Profession.NITWIT);

                        npc.getPersistentDataContainer().set(shopKey, org.bukkit.persistence.PersistentDataType.STRING,
                                        shopName);
                        player.sendMessage(ca.pandaaa.animalquest.utils.Utils.applyFormat("&aShop NPC summoned!"));
                }
        }
        ...
        public void removeShopNpc(Player player) {
                ...
                if (result != null && result.getHitEntity() != null) {
                        org.bukkit.entity.Entity target = result.getHitEntity();
        ...
        public void onNpcClick(PlayerInteractEntityEvent event) {
                if (event.getRightClicked().getPersistentDataContainer().has(shopKey,
                                org.bukkit.persistence.PersistentDataType.STRING)) {
                        event.setCancelled(true);
                        String shopName = event.getRightClicked().getPersistentDataContainer().get(shopKey,
                                        org.bukkit.persistence.PersistentDataType.STRING);
